...
pub trait TenantConnectionProvider: Send + Sync + 'static {
    ...
    fn connection_for(&self, tenant: &TenantId) -> impl Future<Output = Result<DatabaseConnection, TenantError>> + Send;

    /// Get a connection not associated with any specific tenant (admin /
    /// bootstrap operations).
    fn any_connection(&self) -> impl Future<Output = Result<DatabaseConnection, TenantError>> + Send;
}
